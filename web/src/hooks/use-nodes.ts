...
export const NODE_QUERY_KEYS = {
  ...
  list: (userId: string, parentId: string | null) =>
    [...NODE_QUERY_KEYS.lists(), { userId, parentId }] as const,
  details: () => [...NODE_QUERY_KEYS.all, "detail"] as const,
  detail: (userId: string, id: string) =>
    [...NODE_QUERY_KEYS.details(), { userId, id }] as const,
};
  ...
  const rename = useMutation({
    ...
    onSuccess: (response) => {
      ...
      queryClient.setQueryData(
        NODE_QUERY_KEYS.detail(userId, response.id),
        response,
      );
    },
  });
