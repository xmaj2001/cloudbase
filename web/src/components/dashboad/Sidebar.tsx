...
import { useDrivers } from "@/hooks/use-drivers"; // Hook real que já criaste para os drivers
import { useUser } from "@/hooks/use-user";

// Estrutura unificada dos links principais da Sidebar
const NAV_ITEMS = [
  { id: "all", label: "Todos os ficheiros", icon: Home, href: "/storage" },
  { id: "explore", label: "Explorador", icon: FolderTree, href: "/storage/explorer" },
  { id: "drivers", label: "Os Meus Drivers", icon: HardDrive, href: "/storage/drivers" },
  { id: "public", label: "Área Pública", icon: Globe2, href: "/storage/public" },
...
export function SidebarStorage() {
  ...
  const { data: realDrivers = [] } = useDrivers(userId ?? "");

  // Mock provisório enquanto não crias os Spaces Lógicos no backend
  const logicalSpaces = [
    { id: "ls-1", name: "Projecto Alpha", sizeGb: 50, usedGb: 18.4, emoji: "▲" },
    { id: "ls-2", name: "Família Silva", sizeGb: 100, usedGb: 67.1, emoji: "◆" },
  ];

  return (
    <aside className="w-64 shrink-0 border-r border-hairline h-screen sticky top-0 flex flex-col bg-background select-none">
      {/* Brand Logo */}
      <Link href="/storage" className="flex items-center gap-2.5 px-5 h-16 border-b border-hairline">
      ...
      <div className="px-5 mt-4 flex-1 overflow-y-auto no-scrollbar space-y-6">
        <div>
          <div className="flex items-center justify-between mb-2">
            ...
            <Link href="/storage/drivers" className="text-muted-foreground hover:text-foreground transition-colors">
          ...
          <ul className="space-y-1">
            {realDrivers.slice(0, 5).map((d) => {
              ...
              return (
                <li key={d.id} className="flex items-center gap-2.5 text-[13px] py-1 text-muted-foreground font-sans">
                  <Icon className="size-3.5 text-muted-foreground/70 shrink-0" />
                  <span className="truncate text-foreground/90">{d.displayName}</span>
                  {d.status === "SYNCING" && (
            ...
            {realDrivers.length > 5 && (
              <Link href="/storage/drivers" className="text-[11px] font-medium text-muted-foreground hover:underline block pt-1">
            ...
            {logicalSpaces.map((s) => (
              <li key={s.id}>
                <button className="w-full flex items-center gap-2 text-[13px] py-1 hover:text-foreground text-muted-foreground transition-all group text-left">
                  <span className="text-foreground/80 font-mono group-hover:scale-110 transition-transform">{s.emoji}</span>
      ...
      <div className="p-3 border-t border-hairline bg-surface-1/10">
        <Link href="/storage/settings" className="w-full flex items-center gap-3 px-3 py-2 rounded-lg text-sm text-muted-foreground hover:text-foreground hover:bg-surface-2 transition-colors">
