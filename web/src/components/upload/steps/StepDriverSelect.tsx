...
function getFreeSpace(driver: ApiDriver): string {
  if(driver.space.totalSpace === "0") return "∞";
  const free = Number(driver.space.totalSpace) - Number(driver.space.usedSpace);
  return fmtBytes(free) + " disponíveis";
}
function getpct(driver: ApiDriver): number {
  if(driver.space.totalSpace === "0") return 100;
  const pct = (Number(driver.space.usedSpace) / Number(driver.space.totalSpace)) * 100;
  return pct;
}
      ...
      <ul className="space-y-2">
        {drivers.map((d) => {
          ...
          return (
            ...
            >
              <Checkbox checked={isOn} onCheckedChange={() => toggleDriver(d)} />
