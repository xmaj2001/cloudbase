...
export async function seedUser2() {
  ...
  const cloudinary = await prisma.driver.create({
    data: {
      userId: user.id,
      type: 'CLOUDINARY',
  ...
  const oneDrive = await prisma.driver.create({
    data: {
      userId: user.id,
      type: 'ONEDRIVE',
  ...
  const imageFiles = [
    {
      name: 'produto_hero_01.jpg',
      summary: 'Imagem principal do produto em fundo branco, ângulo frontal.',
    },
    {
      name: 'produto_lifestyle_02.jpg',
      summary: 'Produto em contexto de uso real num ambiente moderno.',
    },
    {
      name: 'produto_detalhe_03.jpg',
      summary: 'Close-up com detalhe de textura e materiais.',
    },
    {
      name: 'produto_pack_04.jpg',
      summary: 'Packaging completo com todos os acessórios incluídos.',
    },
  ];
  ...
  for (const file of imageFiles) {
    await prisma.node.create({
      data: {
        ...
        aiConfidence: faker.number.float({
          min: 0.82,
          max: 0.99,
          fractionDigits: 2,
        }),
    ...
    data: [
      {
        ...
        aiConfidence: 0.91,
        aiSummary:
          'Currículo profissional com experiência em design gráfico e UX.',
        status: 'ACTIVE',
      },
      {
        userId: user.id,
        ...
        aiConfidence: 0.88,
        aiSummary:
          'Contrato de prestação de serviços de design com data de renovação em breve.',
        status: 'ACTIVE',
      },
    ],
  });
