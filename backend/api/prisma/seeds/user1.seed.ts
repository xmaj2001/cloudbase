...
export async function seedUser1() {
  ...
  const googleDrive = await prisma.driver.create({
    data: {
      userId: user.id,
      type: 'GOOGLE_DRIVE',
  ...
  const telegram = await prisma.driver.create({
    data: {
      userId: user.id,
      type: 'TELEGRAM',
    ...
    data: [
      {
        ...
        aiConfidence: 0.97,
        aiSummary:
          'Relatório financeiro do Q1 2026 com análise de receitas e despesas operacionais.',
        status: 'ACTIVE',
      },
      {
        userId: user.id,
