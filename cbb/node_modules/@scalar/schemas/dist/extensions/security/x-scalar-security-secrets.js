...
import { typeCommentWithExample } from '../type-comment.js';
export const XScalarSecretToken = object({
    ...
    typeComment: typeCommentWithExample('Persisted OAuth access token (sensitive). Do not export when exporting the document.', { language: 'yaml', body: 'x-scalar-secret-token: eyJhbG...' }),
});
...
export const XScalarSecretHTTP = object({
    ...
    typeComment: typeCommentWithExample('Persisted HTTP basic credentials (sensitive). Do not export when exporting the document.', {
        language: 'yaml',
