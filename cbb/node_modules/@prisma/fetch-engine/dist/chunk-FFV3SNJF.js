...
var chunk_FFV3SNJF_exports = {};
__export(chunk_FFV3SNJF_exports, {
  download: () => download,
...
module.exports = __toCommonJS(chunk_FFV3SNJF_exports);
var import_chunk_5564WBCW = require("./chunk-5564WBCW.js");
var import_chunk_7JLQJWOR = require("./chunk-7JLQJWOR.js");
var import_chunk_IF4T5P2G = require("./chunk-IF4T5P2G.js");
var import_chunk_PPPJF3PE = require("./chunk-PPPJF3PE.js");
var import_chunk_D2GOUA3T = require("./chunk-D2GOUA3T.js");
...
var $ = create$();
var import_fs_extra = (0, import_chunk_2ESYSVXG.__toESM)((0, import_chunk_D2GOUA3T.require_lib)());
...
var import_temp_dir = (0, import_chunk_2ESYSVXG.__toESM)((0, import_chunk_PPPJF3PE.require_temp_dir)());
...
async function download(options) {
  ...
  if (binariesToDownload.length > 0) {
    const cleanupPromise = (0, import_chunk_IF4T5P2G.cleanupCache)();
    let finishBar;
    let setProgress;
    ...
    const promises = binariesToDownload.map((job) => {
      const downloadUrl = (0, import_chunk_D2GOUA3T.getDownloadUrl)({
        channel: "all_commits",
...
async function binaryNeedsToBeDownloaded(job, nativePlatform, version) {
  ...
  if (cachedFile) {
    if (job.skipCacheIntegrityCheck === true) {
      await (0, import_chunk_D2GOUA3T.overwriteFile)(cachedFile, job.targetFilePath);
      return false;
    }
...
async function getCachedBinaryPath({
  ...
  const cacheDir = await (0, import_chunk_D2GOUA3T.getCacheDir)(channel, version, binaryTarget);
  if (!cacheDir) {
    return null;
  }
...
async function downloadBinary(options) {
  ...
  const { sha256, zippedSha256 } = await (0, import_chunk_PPPJF3PE.downloadZip)(downloadUrl, targetFilePath, progressCb);
  if (progressCb) {
    progressCb(1);
  }
...
async function saveFileToCache(job, version, sha256, zippedSha256) {
  const cacheDir = await (0, import_chunk_D2GOUA3T.getCacheDir)(channel, version, job.binaryTarget);
  if (!cacheDir) {
    return;
  }
  ...
  try {
    await (0, import_chunk_D2GOUA3T.overwriteFile)(job.targetFilePath, cachedTargetPath);
    if (sha256 != null) {
