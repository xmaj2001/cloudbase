...
> *Imagine ter acesso a centenas de gigabytes de armazenamento gratuito, distribuído inteligentemente por múltiplas plataformas, gerido por um único sistema, acessível de qualquer dispositivo, controlável pelo WhatsApp, com suporte a streaming de vídeo, partilha segura, troca de ficheiros entre utilizadores, e um SDK completo para developers — tudo isto sem pagar nada para começar.*
>
> **Isso é o CloudBase.**
...
O CloudBase não é mais uma plataforma de armazenamento na nuvem.

É uma **camada de orquestração inteligente** que se liga ao espaço de armazenamento que já existe nas plataformas que já usas — Google Drive, Telegram, OneDrive, MEGA, Cloudinary, Supabase, Firebase, e muito mais — e transforma esse espaço fragmentado e difícil de gerir numa única experiência unificada, poderosa e automatizada.

A ideia central é simples: **o espaço já existe. O que falta é inteligência para o gerir.**

O CloudBase traz essa inteligência.

---

## Índice

- [Porquê o CloudBase existe](#-porquê-o-cloudbase-existe)
...
- [HLS Streaming para Vídeo e Áudio](#-hls-streaming-para-vídeo-e-áudio)
...
- [Rede de Dispositivos — CloudBase Agent](#-rede-de-dispositivos--cloudbase-agent)
- [Transferência Local WiFi e Bluetooth](#-transferência-local-wifi-e-bluetooth)
...
## 🔍 Porquê o CloudBase existe

Hoje, um utilizador comum tem espaço de armazenamento espalhado por cinco ou mais plataformas diferentes:

```
Google Drive    →  15 GB    (quase cheio)
OneDrive        →   5 GB    (esqueceu que existe)
Telegram        →   ∞       (usa só para conversar)
Cloudinary      →  25 GB    (só sabe o que é se for developer)
MEGA            →  20 GB    (criou conta uma vez e nunca mais entrou)
```

O espaço existe. O problema é que está fragmentado, desorganizado, e impossível de gerir em conjunto.

Para os developers, o problema é diferente mas igualmente real: configurar Cloudinary, Supabase, Firebase, e outros providers separadamente para cada projecto é repetitivo, caro à medida que o projecto cresce, e não existe nenhuma solução que una tudo numa única API simples.

O CloudBase resolve ambos os problemas.
...
## 🎬 HLS Streaming para Vídeo e Áudio
...
O algoritmo considera em tempo real:
...
## 🖥️ Rede de Dispositivos — CloudBase Agent
...
## 📡 Transferência Local WiFi e Bluetooth
...
```
Fase 1 — Core Storage                           em desenvolvimento
──────────────────────────────────────────────────────────────────
  Integração Google Drive com múltiplas contas
  Integração Telegram
  Indexação de ficheiros
  Explorer (grelha e lista)
  Upload com routing automático
  Fragmentação com verificação SHA-256

Fase 2 — Partilha e Automação                   planeada
──────────────────────────────────────────────────────────────────
  Sistema de partilha (link, código, QR Code)
  Motor de regras de automação
  Lixeira com retenção configurável
  Collections e validade de ficheiros

Fase 3 — Developer SDK                          planeada
──────────────────────────────────────────────────────────────────
  API Keys e autenticação
  SDK npm (@cloudbase/sdk)
  Cloudinary, Supabase, Firebase, ImageKit, Backblaze B2
  HLS Streaming
  Dashboard de uso de bandwidth

Fase 4 — Devices e Conectividade               planeada
──────────────────────────────────────────────────────────────────
  CloudBase Agent (VPS, PC, Mac, Linux)
  Redirecionamento de transferências
  Transferência WiFi e Bluetooth

Fase 5 — Social e Colaboração                  futura
──────────────────────────────────────────────────────────────────
  Space Pools com reputação
  Troca de ficheiros
  Dead Man's Switch
  Espaço público com moderação
  Bot WhatsApp completo

Fase 6 — IA e Escala                           futura
──────────────────────────────────────────────────────────────────
  Pesquisa semântica
  Classificação automática
  Resumo de documentos e transcrição de áudio
  OCR em imagens
...
Se acreditas que o armazenamento pessoal pode ser radicalmente melhor do que o que existe hoje, e queres ajudar a construir algo que nunca foi feito desta forma — estás no sítio certo.
